...
namespace ls
{
     ...
     */
    UINT8* ls_frame_alloc(UINT32 numBytes);
     ...
     */
    UINT8* ls_frame_alloc_aligned(UINT32 count, UINT32 align);
     ...
     */
    void ls_frame_free(void* data);
    
    /**
     * Frees memory previously allocated with ls_frame_alloc_aligned().
     *
     * @note    Must be called on the same thread the memory was allocated on.
     */
    void ls_frame_free_aligned(void* data);
    ...
    T* ls_frame_alloc()
    {
        return (T*)ls_frame_alloc(sizeof(T));
    }
     ...
     */
    template<class T>
    T* ls_frame_alloc(UINT32 count)
    {
        return (T*)ls_frame_alloc(sizeof(T) * count);
    }
     ...
     */
    template<class T>
    T* ls_frame_new(UINT32 count = 0)
    {
        T* data = ls_frame_alloc<T>(count);
     ...
     */
    template<class T, class... Args>
    T* ls_frame_new(Args &&...args, UINT32 count = 0)
    {
        T* data = ls_frame_alloc<T>(count);
     ...
     */
    template<class T>
    void ls_frame_delete(T* data)
    {
        data->~T();
        
        ls_frame_free((UINT8*)data);
    }
     ...
     */
    template<class T>
    void ls_frame_delete(T* data, UINT32 count)
    {
        for(unsigned int i = 0; i < count; i++)
            data[i].~T();
    ...
    extern THREAD_LOCAL FrameAlloc* _GlobalFrameAlloc;
    ...
    public:
        ...
        {
            return ls_frame_alloc((UINT32)bytes);
        }
        ...
        static void* allocateAligned(size_t bytes, size_t alignment)
        {
            ...
            return ls_frame_alloc_aligned((UINT32)bytes, (UINT32)alignment);
        }
        ...
        static void* allocateAligned16(size_t bytes)
        {
            ...
            return ls_frame_alloc_aligned((UINT32)bytes, 16);
        }
