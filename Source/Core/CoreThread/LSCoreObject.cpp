#include "CoreThread/LSCoreObject.h"
...
namespace ls
{
	...
	CoreObject::~CoreObject() 
	{
		if(!isDestroyed())
		{
			...
			LS_EXCEPT(InternalErrorException, "Destructor called but object is not destroyed. This will result in nasty issues.");
		}

#if DEBUG_MODE
		if(!mThis.expired())
		{
			LS_EXCEPT(InternalErrorException, "Shared pointer to this object still has active references but " \
	...
	void CoreObject::destroy()
	{
		...
		{
			assert(LS_THREAD_CURRENT_ID != CoreThread::instance().getCoreThreadId() && "Cannot destroy sim thead object from core thread.");
	...
	void CoreObject::initialize()
	{
		...
		if (mCoreSpecific != nullptr)
		{
			if (requiresInitOnCoreThread())
			{
				...
				assert(LS_THREAD_CURRENT_ID != CoreThread::instance().getCoreThreadId() && "Cannot initialize sim thread object from core thread.");
