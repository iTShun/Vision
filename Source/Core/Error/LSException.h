...
namespace ls
{
	...
	/**
	 * Base class for all LSEngine exceptions.
	 *
	 * @note	LSEngine doesn't perform exception handling, but these classes remain here in case others wish to enable them.
	 */
	...
	public:
		...
		virtual const String& getFullDescription() const
		{
			if (mFullDesc.empty())
			{
				...
				desc << "LSENGINE EXCEPTION(" << mTypeName << "): "
					<< mDescription
					<< " in " << mSource;
	 ...
	 * exception.
	 */
	// LSEngine doesn't actually use exceptions, so we just emulate the unhandled exception handler by crashing the application.
#ifndef LS_EXCEPT
#define LS_EXCEPT(type, desc)																\
	{																						\
