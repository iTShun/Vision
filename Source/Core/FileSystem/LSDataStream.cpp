...
namespace ls
{
	...
	String DataStream::getAsString()
	{
		...
		auto tempBuffer = ls_stack_alloc<std::stringstream::char_type>((UINT32)bufSize);
	...
	{
		mData = mPos = (UINT8*)ls_alloc((UINT32)size);
		mSize = size;
		mEnd = mData + mSize;
		...
		: DataStream(READ | WRITE), mData(nullptr)
	{
		...
		mData = (UINT8*)ls_alloc((UINT32)mSize);
		mPos = mData;
	...
	{
		if (!copyData)
			return ls_shared_ptr_new<MemoryDataStream>(mData, mSize, false);

		return ls_shared_ptr_new<MemoryDataStream>(*this);
	}
		...
		: DataStream(accessMode), mPath(path), mFreeOnClose(freeOnClose)
	{
		...
		{
			mode |= std::ios::out;
			mFStream = ls_shared_ptr_new<std::fstream>();
			mFStream->open(path.toPlatformString().c_str(), mode);
			mInStream = mFStream;
		}
		else
		{
			mFStreamRO = ls_shared_ptr_new<std::ifstream>();
	...
	{
		return ls_shared_ptr_new<FileDataStream>(mPath, (AccessMode)getAccessMode(), true);
	}
