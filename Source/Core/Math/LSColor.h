...
namespace ls 
{
	...
	public:
		...
		static const Color LSEngineOrange;
	...
	LS_ALLOW_MEMCPY_SERIALIZATION(Color);
	/** @endcond */
...
template<> 
struct hash<ls::Color>
{
	size_t operator()(const ls::Color& color) const
	{
		size_t hash = 0;
		ls::hash_combine(hash, color.r);
		ls::hash_combine(hash, color.g);
		ls::hash_combine(hash, color.b);
		ls::hash_combine(hash, color.a);
