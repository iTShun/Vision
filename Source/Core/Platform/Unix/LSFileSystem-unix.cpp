...
#if PLATFORM_LINUX || PLATFORM_OSX || PLATFORM_IOS
...
#define HANDLE_PATH_ERROR(path__, errno__) \
    LOGERR(String(FUNCTION) + ": " + (path__) + ": " + (strerror(errno__)));
...
namespace ls
{
    ...
    void FileSystem::moveFile(const Path& oldPath, const Path& newPath)
    {
        ...
        {
            // Cross-filesystem copy is likely needed (for example, /tmp to LSEngine install dir while copying assets)
    ...
    SPtr<DataStream> FileSystem::openFile(const Path& path, bool readOnly)
    {
        ...
        return ls_shared_ptr_new<FileDataStream>(path, accessMode, true);
    }
    
    SPtr<DataStream> FileSystem::createAndOpenFile(const Path& path)
    {
        return ls_shared_ptr_new<FileDataStream>(path, DataStream::AccessMode::WRITE, true);
    }
    ...
    {
        char *buffer = ls_newN<char>(PATH_MAX);
...
#endif // PLATFORM_LINUX || PLATFORM_OSX || PLATFORM_IOS
