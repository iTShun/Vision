...
namespace ls
{
	...
	WString win32_getCurrentDirectory()
	{
		...
		if (len > 0)
		{
			wchar_t* buffer = (wchar_t*)ls_alloc(len * sizeof(wchar_t));
	...
	SPtr<DataStream> FileSystem::openFile(const Path& fullPath, bool readOnly)
	{
		...
		return ls_shared_ptr_new<FileDataStream>(fullPath, accessMode, true);
	}

	SPtr<DataStream> FileSystem::createAndOpenFile(const Path& fullPath)
	{
		return ls_shared_ptr_new<FileDataStream>(fullPath, DataStream::AccessMode::WRITE, true);
	}
