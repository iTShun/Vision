...
#define LS_FLAGS_OPERATORS(Enum) LS_FLAGS_OPERATORS_EXT(Enum, UINT32)

/** Defines global operators for a Flags<Enum, Storage> implementation. */
#define LS_FLAGS_OPERATORS_EXT(Enum, Storage)																   \
...
namespace std
{
	...
	struct hash<ls::Flags<Enum, Storage>>
	{
		size_t operator()(const ls::Flags<Enum, Storage>& key) const
		{
			return (Storage)key;
		}
	};
