...
namespace ls
{
	...
	private:
		...
		MemBlock* allocBlock(UINT32 wantedSize)
		{
			...
			if (newBlock == nullptr)
			{
				UINT8* data = (UINT8*)reinterpret_cast<UINT8*>(ls_alloc(blockSize + sizeof(MemBlock)));
	...
	public:
		 ...
		 */
		static LS_UTILITY_EXPORT void beginThread();
		 ...
		 */
		static LS_UTILITY_EXPORT void endThread();

		/** @copydoc MemStackInternal::alloc() */
		static LS_UTILITY_EXPORT UINT8* alloc(UINT32 amount);

		/** @copydoc MemStackInternal::dealloc() */
		static LS_UTILITY_EXPORT void deallocLast(UINT8* data);

	private:
		static LS_THREADLOCAL MemStackInternal<1024 * 1024>* ThreadMemStack;
	};
	...
	/** @copydoc MemStackInternal::alloc() */
	inline void* ls_stack_alloc(UINT32 amount)
	{
	 ...
	 */
	template<class T>
	T* ls_stack_alloc(UINT32 amount)
	{
	 ...
	 */
	template<class T>
	T* ls_stack_new(UINT32 count = 0)
	{
		T* data = ls_stack_alloc<T>(count);
	...
	T* ls_stack_new(Args &&...args, UINT32 count = 0)
	{
		T* data = ls_stack_alloc<T>(count);
	 ...
	 */
	template<class T>
	void ls_stack_delete(T* data, UINT32 count)
	{
		for(unsigned int i = 0; i < count; i++)
			data[i].~T();
	...
	inline void ls_stack_delete(void* data, UINT32 count)
	{
		MemStack::deallocLast((UINT8*)data);
	}

	/** @copydoc MemStackInternal::dealloc() */
	inline void ls_stack_free(void* data)
	{
	...
	struct StackMemory
	{
		...
		/*
		 * This ensures that the result of ls_managed_stack_alloc() doesn't get passed to a function call as a temporary, 
		 * or immediately assigned as a T*. Instead, the user of this class is forced to deal with this class as itself, 
		 * when handling the return value of ls_managed_stack_alloc() preventing an immediate (and erroneous) call to 
		 * ls_stack_free().
		 */
		...
		~StackMemory()
		{
			...
			{
				if(mCount >= 1)
					ls_stack_delete(mPtr, (UINT32)mCount);
				else
					ls_stack_free(mPtr);
			}
		}
	...
	/** 
	 * Same as ls_stack_alloc() except the returned object takes care of automatically cleaning up when it goes out of 
	 * scope. 
	 */
	inline StackMemory<void> ls_managed_stack_alloc(UINT32 amount)
	{
		return StackMemory<void>(ls_stack_alloc(amount));
	}
	 ...
	 */
	template<class T>
	StackMemory<T> ls_managed_stack_alloc()
	{
		return StackMemory<T>(ls_stack_alloc<T>());
	}
	 ...
	 */
	template<class T>
	StackMemory<T> ls_managed_stack_alloc(UINT32 amount)
	{
		return StackMemory<T>(ls_stack_alloc<T>(amount));
	}

	/** 
	 * Same as ls_stack_new() except the returned object takes care of automatically cleaning up when it goes out of 
	 * scope. 
	 */
	template<class T>
	StackMemory<T> ls_managed_stack_new(size_t count = 1)
	{
		return StackMemory<T>(ls_stack_new<T>(count), count);
	}
	 ...
	 */
	template<class T, class... Args>
	StackMemory<T> ls_managed_stack_new(Args && ... args, size_t count = 1)
	{
		return StackMemory<T>(ls_stack_new<T>(std::forward<Args>(args)..., count), count);
	}
	...
	*/
	...
	public:
		...
		{
			return ls_stack_alloc((UINT32)bytes);
		}
