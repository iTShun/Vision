	...
	private:
		...
		void realloc(uint32_t numBits)
		{
			...
			if(numBits != mMaxBits)
			{
				...
				auto buffer = ls_allocN<uint32_t>(numDwords);
				if(mData)
				{
...
namespace std
{
	template <> inline void swap(ls::BitReference& lhs, ls::BitReference& rhs)
	{
		const bool temp = lhs;
		lhs = rhs;
		rhs = temp;
	...
	inline void swap(ls::BitReference&& lhs, ls::BitReference&& rhs)
	{
		const bool temp = lhs;
		lhs = rhs;
		rhs = temp;
