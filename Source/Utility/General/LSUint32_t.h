...
#define HALF_FLOAT_ZERO UINT16_C(0)
#define HALF_FLOAT_HALF UINT16_C(0x3800)
#define HALF_FLOAT_ONE  UINT16_C(0x3c00)
#define HALF_FLOAT_TWO  UINT16_C(0x4000)
...
namespace ls
{
	...
	/// Count number of leading non zeros.
	inline uint32_t uint32_cntlnz(uint32_t _val)
	{
#if COMPILER_GCC || COMPILER_CLANG
	...
	inline uint32_t uint64_cntbits_ref(uint64_t _val)
	{
		...
		const uint32_t total = ls::uint32_cntbits(lo)
			+ ls::uint32_cntbits(hi);
		return total;
	}
	...
	/// Count number of leading non zeros.
	inline uint32_t uint64_cntlnz(uint64_t _val)
	{
#if COMPILER_GCC || COMPILER_CLANG
