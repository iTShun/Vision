...
namespace ls
{
	...
	String LS_UTILITY_EXPORT md5(const WString& source);

	/**	Generates an MD5 hash string for the provided source string. */
	String LS_UTILITY_EXPORT md5(const String& source);

	/** Sets contents of a struct to zero. */
	template<class T>
	void ls_zero_out(T& s)
	{
	...
	template<class T>
	void ls_zero_out(T* arr, size_t count)
	{
		assert(arr != nullptr);
	...
	template<class T, size_t N>
	void ls_copy(T(&dst)[N], T(&src)[N], size_t count)
	{
	...
	/** Copies the contents of one array to another. Automatically accounts for array element size. */
	template<class T>
	void ls_copy(T* dst, T* src, size_t count)
	{
		std::memcpy(dst, src, sizeof(T) * count);
	}
	...
	constexpr size_t ls_size(const T (&array)[N])
	{
		return N;
	}
	...
	bool ls_swap_and_erase(Vector<T, A>& container, const typename Vector<T, A>::iterator iter)
	{
