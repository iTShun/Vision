...
/*        }  */
	...
	String PlatformUtility::convertCaseUTF8(const String& input, bool toUpper)
	{
		...
		auto uStr = ls_stack_alloc<UChar>((UINT32)bufferLen);
		int32_t uStrLen = 0;
		...
		auto convertedUStr = ls_stack_alloc<UChar>((UINT32) bufferLen);
		...
		auto outputStr = ls_stack_alloc<char>(bufferLen);
		...
		ls_stack_free(outputStr);
		ls_stack_free(convertedUStr);
		ls_stack_free(uStr);
