...
#include "Reflection/LSRTTIPlainField.h"
#include "Reflection/LSRTTIReflectableField.h"
#include "Reflection/LSRTTIReflectablePtrField.h"
#include "Reflection/LSRTTIManagedDataBlockField.h"
#include "Reflection/LSIReflectable.h"
#include "Serialization/LSBinaryDiff.h"
...
namespace ls
{
	 ...
	 */
	 ...
	 /**
	  * Starts definitions for member fields within a RTTI type. Follow this with calls to LS_RTTI_MEMBER* calls, and finish by
	  * calling LS_END_RTTI_MEMBERS.
	  */
#define LS_BEGIN_RTTI_MEMBERS																	\
	...
	typedef META_FirstEntry
	   ...
	   */
#define LS_RTTI_MEMBER_PLAIN(name, id)															\
	META_Entry_##name;																			\
																								\
	...
	typedef META_NextEntry_##name
	   ...
	   /** Same as LS_RTTI_MEMBER_PLAIN, but allows you to specify separate names for the field name and the member variable. */
#define LS_RTTI_MEMBER_PLAIN_NAMED(name, field, id)												\
	META_Entry_##name;																			\
																								\
 ...
 */
#define LS_RTTI_MEMBER_PLAIN_ARRAY(name, id)													\
	META_Entry_##name;																			\
																								\
 ...
 /** 
  * Same as LS_RTTI_MEMBER_PLAIN_ARRAY, but allows you to specify separate names for the field name and the member variable.
  */
#define LS_RTTI_MEMBER_PLAIN_ARRAY_NAMED(name, field, id)													\
	META_Entry_##name;																			\
																								\
 ...
 */
#define LS_RTTI_MEMBER_REFL(name, id)															\
	META_Entry_##name;																			\
																								\
	decltype(OwnerType::name)& get##name(OwnerType* obj) { return obj->name; }					\
	void set##name(OwnerType* obj, decltype(OwnerType::name)& val) { obj->name = val; }			\
...
/** Same as LS_RTTI_MEMBER_REFL, but allows you to specify separate names for the field name and the member variable. */
#define LS_RTTI_MEMBER_REFL_NAMED(name, field, id)												\
	META_Entry_##name;																			\
																								\
	decltype(OwnerType::field)& get##name(OwnerType* obj) { return obj->field; }				\
	void set##name(OwnerType* obj, decltype(OwnerType::field)& val) { obj->field = val; }		\
 ...
 * @p id.
 */
#define LS_RTTI_MEMBER_REFL_ARRAY(name, id)														\
	META_Entry_##name;																			\
																								\
	std::common_type<decltype(OwnerType::name)>::type::value_type& get##name(OwnerType* obj, UINT32 idx) { return obj->name[idx]; }					\
	void set##name(OwnerType* obj, UINT32 idx, std::common_type<decltype(OwnerType::name)>::type::value_type& val) { obj->name[idx] = val; }		\
...
/** 
 * Same as LS_RTTI_MEMBER_REFL_ARRAY, but allows you to specify separate names for the field name and the member variable. 
 */
#define LS_RTTI_MEMBER_REFL_ARRAY_NAMED(name, field, id)										\
	META_Entry_##name;																			\
																								\
	std::common_type<decltype(OwnerType::field)>::type::value_type& get##name(OwnerType* obj, UINT32 idx) { return obj->field[idx]; }				\
	void set##name(OwnerType* obj, UINT32 idx, std::common_type<decltype(OwnerType::field)>::type::value_type& val) { obj->field[idx] = val; }		\
 ...
 */
#define LS_RTTI_MEMBER_REFLPTR(name, id)														\
	META_Entry_##name;																			\
																								\
	decltype(OwnerType::name) get##name(OwnerType* obj) { return obj->name; }					\
	void set##name(OwnerType* obj, decltype(OwnerType::name) val) { obj->name = val; }			\
...
/** Same as LS_RTTI_MEMBER_REFLPTR, but allows you to specify separate names for the field name and the member variable. */
#define LS_RTTI_MEMBER_REFLPTR_NAMED(name, field, id)											\
	META_Entry_##name;																			\
																								\
	decltype(OwnerType::field) get##name(OwnerType* obj) { return obj->field; }					\
	void set##name(OwnerType* obj, decltype(OwnerType::field) val) { obj->field = val; }		\
 ...
 * The type of the member must be a valid reflectable pointer type. Each field must specify a unique ID for @p id.
 */
#define LS_RTTI_MEMBER_REFLPTR_ARRAY(name, id)													\
	META_Entry_##name;																			\
																								\
	std::common_type<decltype(OwnerType::name)>::type::value_type get##name(OwnerType* obj, UINT32 idx) { return obj->name[idx]; }				\
	void set##name(OwnerType* obj, UINT32 idx, std::common_type<decltype(OwnerType::name)>::type::value_type val) { obj->name[idx] = val; }		\
 ...
 /**
  * Same as LS_RTTI_MEMBER_REFLPTR_ARRAY, but allows you to specify separate names for the field name and the member 
  * variable. 
  */
#define LS_RTTI_MEMBER_REFLPTR_ARRAY_NAMED(name, field, id)										\
	META_Entry_##name;																			\
																								\
	std::common_type<decltype(OwnerType::field)>::type::value_type get##name(OwnerType* obj, UINT32 idx) { return obj->field[idx]; }				\
	void set##name(OwnerType* obj, UINT32 idx, std::common_type<decltype(OwnerType::field)>::type::value_type val) { obj->field[idx] = val; }		\
...
/** Ends definitions for member fields with a RTTI type. Must follow LS_BEGIN_RTTI_MEMBERS. */
#define LS_END_RTTI_MEMBERS																		\
	META_LastEntry;																				\
																								\
	 ...
	 *	- Plain types - All types defined in LSRTTIField.h, mostly native types and POD (plain old data) structs. Data is parsed byte by byte.  
	 ...
	 */
	class LS_UTILITY_EXPORT RTTITypeBase
	{
	public:
	...
	protected:
		...
		void addPlainField(const String& name, UINT32 uniqueId, 
			...
			UINT64 flags = 0)
		{
			static_assert((std::is_base_of<ls::RTTIType<Type, BaseType, MyRTTIType>, InterfaceType>::value), 
				"Class with the get/set methods must derive from ls::RTTIType.");

			static_assert(!(std::is_base_of<ls::IReflectable, DataType>::value), 
				"Data type derives from IReflectable but it is being added as a plain field.");

			auto newField = ls_new<RTTIPlainField<InterfaceType, DataType, ObjectType>>();
		...
		{
			static_assert((std::is_base_of<ls::IReflectable, DataType>::value), 
				"Invalid data type for complex field. It needs to derive from ls::IReflectable.");

			auto newField = ls_new<RTTIReflectableField<InterfaceType, DataType, ObjectType>>();
			newField->initSingle(name, uniqueId, getter, setter, flags);
			addNewField(newField);
		}
			...
			void (InterfaceType::*setter)(ObjectType*, SPtr<DataType>), UINT64 flags = 0)
		{
			...
			auto newField = ls_new<RTTIReflectablePtrField<InterfaceType, DataType, ObjectType>>();
			newField->initSingle(name, uniqueId, getter, setter, flags);
			addNewField(newField);
		}
		...
		{
			auto newField = ls_new<RTTIManagedDataBlockField<InterfaceType, UINT8*, ObjectType>>();
			newField->initSingle(name, uniqueId, getter, setter, flags);
			addNewField(newField);
		}	
	};
	...
	struct LS_UTILITY_EXPORT SerializationContext : IReflectable
	{
		UINT32 flags = 0;
	...
	{
		static_assert((std::is_base_of<ls::IReflectable, T>::value), 
			"Invalid data type for type checking. It needs to derive from ls::IReflectable.");
	...
	LS_UTILITY_EXPORT SPtr<IReflectable> rtti_create(UINT32 rttiId);
