...
namespace ls
{
	 ...
	 */
	class LS_CORE_EXPORT CoreObject
	{
	protected:
	...
	SPtr<Type> ls_core_ptr_new(Args &&...args)
	{
		return SPtr<Type>(ls_new<Type, MainAlloc>(std::forward<Args>(args)...),
	...
	SPtr<Type> ls_core_ptr_new(Args &&...args)
	{
		return SPtr<Type>(ls_new<Type, GenAlloc>(std::forward<Args>(args)...),
	...
	SPtr<Type> ls_core_ptr(Type* data)
	{
