#include "General/LSCompression.h"
...
namespace ls
{
	...
	public:
			...
			: mStream(stream)
		{
			...
			if (mStream->isFile())
				mReadBuffer = (char*)ls_alloc(2048);
		}
	...
	public:
		...
		void Append(const char* data, size_t n) override
		{
			...
			{
				BufferPiece piece;
				piece.buffer = (char*)ls_alloc((UINT32)n);
				piece.size = n;
		...
		{
			BufferPiece piece;
			piece.buffer = (char*)ls_alloc((UINT32)len);
			piece.size = 0;
		...
		{
			BufferPiece piece;
			piece.buffer = (char*)ls_alloc((UINT32)desired_size_hint);
			piece.size = 0;
		...
		SPtr<MemoryDataStream> GetOutput()
		{
			...
			SPtr<MemoryDataStream> ds = ls_shared_ptr_new<MemoryDataStream>(totalSize);
			for (auto& entry : mBufferPieces)
