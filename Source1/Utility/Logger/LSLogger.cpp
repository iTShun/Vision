...
#include "Logger/LSBitmapWriter.h"
...
namespace ls
{
	void Logger::logLogger(const String& msg)
	{
	...
	void Logger::log(const String& msg, UINT32 channel)
	{
		mLog.logMsg(msg, channel);
		logToIDEConsole(msg);
	}

	void Logger::writeAsBMP(UINT8* rawPixels, UINT32 bytesPerPixel, UINT32 width, UINT32 height, const Path& filePath,
		bool overwrite) const
	{
		if(FileSystem::isFile(filePath))
		{
			if(overwrite)
				...
				LS_EXCEPT(FileNotFoundException, "File already exists at specified location: " + filePath.toString());
		}
		...
		UINT8* bmpBuffer = ls_newN<UINT8>(bmpDataSize);
		...
		ls_deleteN(bmpBuffer, bmpDataSize);
	}

	void Logger::_triggerCallbacks()
	{
		LogEntry entry;
	...
	void Logger::saveLog(const Path& path) const
	{
...
<body>
	...
	<tbody>
		...
		stream << "<p>Banshee version: " << LS_VERSION_MAJOR << "." << LS_VERSION_MINOR << "</p>\n";
