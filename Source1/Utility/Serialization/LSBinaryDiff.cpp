...
#include "Serialization/LSBinaryCloner.h"
		...
		const SPtr<SerializedInstance>& newData, ObjectMap& objectMap)
	{
		...
		case SerializableFT_DataBlock:
		{
			...
			if (!isModified)
			{
				...
				{
					orgStreamData = (UINT8*)ls_stack_alloc(orgFieldData->size);
				...
				{
					newStreamData = (UINT8*)ls_stack_alloc(newFieldData->size);
				...
				if (newFieldData->stream->isFile())
					ls_stack_free(newStreamData);

				if (orgFieldData->stream->isFile())
					ls_stack_free(orgStreamData);
			}
			...
			for (auto& newEntry : subObject.entries)
			{
					...
					if (newEntryData != nullptr && orgEntryData != nullptr)
					{
						for (auto& arrayEntryPair : newArrayData->entries)
						{
							...
							if (arrayModification != nullptr)
							{
								...
								{
									serializedArray = ls_shared_ptr_new<SerializedArray>();
				...
				else
				{
					...
					else if (newEntryData == nullptr)
					{
						...
						{
						case SerializableFT_Plain:
							modification = ls_shared_ptr_new<SerializedField>();
							break;
						case SerializableFT_DataBlock:
							modification = ls_shared_ptr_new<SerializedDataBlock>();
							break;
						default:
							break;
						}
				...
				{
					if (output == nullptr)
						output = ls_shared_ptr_new<SerializedObject>();
