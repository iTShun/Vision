...
#define LS_TEST_ASSERT(expr) assertment((expr), __FUNCTION__, __FILE__, __LINE__); 

/** Tests if condition is true, and reports unit test failure with a message if it fails. */
#define LS_TEST_ASSERT_MSG(expr, msg) assertment((expr), msg, __FILE__, __LINE__); 
	 ...
	 */
	class LS_UTILITY_EXPORT TestSuite
	{
	public:
	...
	public:
		...
		{
			static_assert((std::is_base_of<TestSuite, T>::value), "Invalid test suite type. It needs to derive from ls::TestSuite.");

			return std::static_pointer_cast<TestSuite>(ls_shared_ptr_new<T>());
		}
...
#define LS_ADD_TEST(func) addTest(static_cast<Func>(&func), #func);
