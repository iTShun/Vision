...
namespace ls
{
	...
	class LS_UTILITY_EXPORT AsyncOpSyncData
	{
	public:
		Mutex mMutex;
		Signal mCondition;
	 ...
	 * memory storage.
	 */
	struct LS_UTILITY_EXPORT AsyncOpEmpty {};
	 ...
	 */
	class LS_UTILITY_EXPORT AsyncOp
	{
	private:
		struct AsyncOpData
		{
	...
	public:
		AsyncOp()
			:mData(ls_shared_ptr_new<AsyncOpData>())
		{ }
		...
		AsyncOp(const SPtr<AsyncOpSyncData>& syncData)
			:mData(ls_shared_ptr_new<AsyncOpData>()), mSyncData(syncData)
		{ }
...
#if DEBUG_MODE
			if(!hasCompleted())
				LS_EXCEPT(InternalErrorException, "Trying to get AsyncOp return value but the operation hasn't completed.");
#endif
