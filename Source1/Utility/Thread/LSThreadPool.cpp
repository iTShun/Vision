...
namespace ls
{
	...
	{
		mThread = ls_new<Thread>(std::bind(&PooledThread::run, this));
	...
	static void tryFunc(std::function<void()>& func)
	{
#if PLATFORM_WINDOWS
		__try
		{
	...
	PooledThread* ThreadPool::getThread(const String& name)
	{
		...
		if(mThreads.size() >= mMaxCapacity)
			LS_EXCEPT(InvalidStateException, "Unable to create a new thread in the pool because maximum capacity has been reached.");
