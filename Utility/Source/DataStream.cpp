...
namespace Utility
{
	...
	String DataStream::getAsString()
	{
		...
		auto tempBuffer = StackAlloc<std::stringstream::char_type>((UINT32)bufSize);
	...
	{
		mData = mPos = (UINT8*)Alloc((UINT32)size);
		mSize = size;
		mEnd = mData + mSize;
		...
		: DataStream(READ | WRITE), mData(nullptr)
	{
		...
		mData = (UINT8*)Alloc((UINT32)mSize);
		mPos = mData;
	...
	{
		if (!copyData)
			return SharedPtrNew<MemoryDataStream>(mData, mSize, false);

		return SharedPtrNew<MemoryDataStream>(*this);
	}
		...
		: DataStream(accessMode), mPath(path), mFreeOnClose(freeOnClose)
	{
		...
		{
			mode |= std::ios::out;
			mFStream = SharedPtrNew<std::fstream>();
			mFStream->open(path.toPlatformString().c_str(), mode);
			mInStream = mFStream;
		}
		else
		{
			mFStreamRO = SharedPtrNew<std::ifstream>();
	...
	{
		return SharedPtrNew<FileDataStream>(mPath, (AccessMode)getAccessMode(), true);
	}
