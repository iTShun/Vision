...
namespace Utility
{
	...
	void FrameAllocator::clear()
	{
		...
		else
		{
			...
			if (mTotalAllocBytes.load() > 0)
				EXCEPT(InvalidStateException, "Not all frame allocated bytes were properly released.");
#endif
	...
	FrameAllocator::MemBlock* FrameAllocator::allocBlock(UINT32 wantedSize)
	{
		...
		if (newBlock == nullptr)
		{
			...
			UINT8* data = (UINT8*)reinterpret_cast<UINT8*>(AllocAligned(blockSize + sizeof(MemBlock) + alignOffset, 16));
	...
	void FrameAllocator::setOwnerThread(ThreadId thread)
	{
	}

	THREAD_LOCAL FrameAllocator* _GlobalFrameAllocator = nullptr;

	FrameAllocator& gFrameAllocator()
	{
	...
	UINT8* FrameAlloc(UINT32 numBytes)
	{
		return gFrameAllocator().alloc(numBytes);
	}

	UINT8* FrameAllocAligned(UINT32 count, UINT32 align)
	{
		return gFrameAllocator().allocAligned(count, align);
	}
	...
	void FrameFreeAligned(void* data)
	{
		gFrameAllocator().free((UINT8*)data);
	}
