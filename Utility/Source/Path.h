...
namespace Utility
{
	...
	private:
		friend struct ::std::hash<Utility::Path>;
...
namespace std
{
	...
	template<>
	struct hash<Utility::Path>
	{
		size_t operator()(const Utility::Path& path) const
		{
			size_t hash = 0;
			Utility::hash_combine(hash, path.mFilename);
			Utility::hash_combine(hash, path.mDevice);
			Utility::hash_combine(hash, path.mNode);

			for (auto& dir : path.mDirectories)
				Utility::hash_combine(hash, dir);
