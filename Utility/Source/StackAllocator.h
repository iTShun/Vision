...
namespace Utility
{
	...
	private:
		...
		MemBlock* allocBlock(UINT32 wantedSize)
		{
			...
			if (newBlock == nullptr)
			{
				UINT8* data = (UINT8*)reinterpret_cast<UINT8*>(Alloc(blockSize + sizeof(MemBlock)));
	...
	public:
		 ...
		 */
		static UTILITY_EXPORT void endThread();

		/** @copydoc MemStackInternal::alloc() */
		static UTILITY_EXPORT UINT8* alloc(UINT32 amount);

		/** @copydoc MemStackInternal::dealloc() */
		static UTILITY_EXPORT void deallocLast(UINT8* data);
	 ...
	 */
	 ...
	 /** @copydoc MemStackInternal::alloc() */
	inline void* StackAlloc(UINT32 amount)
	{
	...
	T* StackNew(Args &&...args, UINT32 count = 0)
	{
		T* data = StackAlloc<T>(count);
	 ...
	 */
	template<class T>
	void StackDelete(T* data, UINT32 count)
	{
		for (unsigned int i = 0; i < count; i++)
			data[i].~T();
	...
	inline void StackDelete(void* data, UINT32 count)
	{
		MemStack::deallocLast((UINT8*)data);
	}

	/** @copydoc MemStackInternal::dealloc() */
	inline void StackFree(void* data)
	{
	...
	struct StackMemory
	{
		...
		/*
		 * This ensures that the result of ManagedStackAlloc() doesn't get passed to a function call as a temporary,
		 * or immediately assigned as a T*. Instead, the user of this class is forced to deal with this class as itself,
		 * when handling the return value of ManagedStackAlloc() preventing an immediate (and erroneous) call to
		 * StackFree().
		 */
		...
		~StackMemory()
		{
			...
			{
				if (mCount >= 1)
					StackDelete(mPtr, (UINT32)mCount);
				else
					StackFree(mPtr);
			}
		}
	 ...
	 * scope.
	 */
	inline StackMemory<void> ManagedStackAlloc(UINT32 amount)
	{
		return StackMemory<void>(StackAlloc(amount));
	}
	 ...
	 */
	template<class T>
	StackMemory<T> ManagedStackAlloc()
	{
		return StackMemory<T>(StackAlloc<T>());
	}
	 ...
	 */
	template<class T>
	StackMemory<T> ManagedStackAlloc(UINT32 amount)
	{
		return StackMemory<T>(StackAlloc<T>(amount));
	}
	 ...
	 */
	template<class T>
	StackMemory<T> ManagedStackNew(size_t count = 1)
	{
		return StackMemory<T>(StackNew<T>(count), count);
	}
	 ...
	 */
	template<class T, class... Args>
	StackMemory<T> ManagedStackNew(Args && ... args, size_t count = 1)
	{
		return StackMemory<T>(StackNew<T>(std::forward<Args>(args)..., count), count);
	}
	...
	*/
	template<>
	class MemoryAllocator<StackAllocator>
	{
	public:
