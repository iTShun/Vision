...
namespace Utility
{
	...
	T UTF32To8(char32_t input, T output, UINT32 maxElems, char invalidChar = 0)
	{
		...
		switch (numBytes)
		{
		case 4: bytes[3] = (char)((input | 0x80) & 0xBF); input >>= 6; FALLTHROUGH;
		case 3: bytes[2] = (char)((input | 0x80) & 0xBF); input >>= 6; FALLTHROUGH;
		case 2: bytes[1] = (char)((input | 0x80) & 0xBF); input >>= 6; FALLTHROUGH;
		case 1: bytes[0] = (char)(input | headers[numBytes]); FALLTHROUGH;
		default: break;
		}
